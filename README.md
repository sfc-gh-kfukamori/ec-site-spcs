...
### 1. SPCS OAuth トークン認証 (`backend/snowflake_client.py`)

SPCS コンテナ内では `/snowflake/session/token` にOAuth トークンが自動的にマウントされます。このファイルの有無で SPCS 環境かローカル環境かを自動判定し、接続方法を切り替えています。

```python
token_path = "/snowflake/session/token"
if os.path.exists(token_path):
    # SPCS 環境: OAuth トークンで接続
    token = open(token_path).read().strip()
    snowflake.connector.connect(authenticator="oauth", token=token, ...)
else:
    # ローカル環境: パスワードで接続
    snowflake.connector.connect(user=..., password=..., ...)
```

これにより、同一コードで SPCS 上でもローカルでも動作します。
...
### 4. マルチステージ Docker ビルド (`Dockerfile`)
...
### 5. 画像フォールバック (`frontend/src/components/ProductImage.tsx`)
...
### 6. パスワードハッシュの整合性 (`backend/main.py` / `setup_spcs.sql`)
